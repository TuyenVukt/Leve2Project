...
public class CategoryController extends HttpServlet {
    ...
    //trong các hàm này rs được ép kiểu từ String sang đối tượng JsonResult để thuận tiện cho các hàm ajax sau này
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (pathInfor.indexOf("/find-all") == 0) {
        ...
        } else if (pathInfor.indexOf("/find-by-name") == 0) {
            ...
            try {
                ...
                e.printStackTrace();
                rs = jsonResult.jsonFail("find by name error");
            }
            response.getWriter().write(rs);
        } else if (pathInfor.indexOf("/sort-asc") == 0) {
            try {
                List<Category> categoryList = categoryService.sortByName(true);
                rs = jsonResult.jsonSuccess(categoryList);
            } catch (Exception e) {
                e.printStackTrace();
//                rs = "find all error";
                rs = jsonResult.jsonFail("sort asc error");
            }
            response.getWriter().write(rs);
        } else if (pathInfor.indexOf("/sort-desc") == 0) {
            try {
                List<Category> categoryList = categoryService.sortByName(false);
                rs = jsonResult.jsonSuccess(categoryList);
            } catch (Exception e) {
                e.printStackTrace();
//                rs = "find all error";
                rs = jsonResult.jsonFail("sort desc error");
            }
            response.getWriter().write(rs);
        } else {
